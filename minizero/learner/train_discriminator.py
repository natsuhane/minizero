...
class DiscriminatorDataLoader:
    def __init__(self, conf_file_name):
        ...
        # allocate memory
        self.features = np.zeros(py.get_batch_size() * py.get_iig_discriminator_feature_channels() * py.get_nn_input_channel_height() * py.get_nn_input_channel_width(), dtype=np.float32)
        self.labels = np.zeros(py.get_batch_size(), dtype=np.float32)
    ...
    def sample_data(self, device='cpu'):
        self.data_loader.sample_discriminator_data(self.features, self.labels)
        features = torch.FloatTensor(self.features).view(py.get_batch_size(), py.get_iig_discriminator_feature_channels(), py.get_nn_input_channel_height(), py.get_nn_input_channel_width()).to(device)
        labels = torch.FloatTensor(self.labels).view(py.get_batch_size(), 1).to(device)
...
class Model:
    ...
    def load_model(self, training_dir, model_file):
        ...
        self.network = create_network(py.get_game_name(),
                                      py.get_iig_discriminator_feature_channels(),
        ...
        if model_file:
            snapshot = torch.load(f"{training_dir}/discriminator_model/{model_file}", map_location=torch.device('cpu'))
    ...
    def save_model(self, training_dir):
        ...
        torch.save(snapshot, f"{training_dir}/discriminator_model/weight_iter_{self.training_step}.pkl")
        torch.jit.script(self.network.module).save(f"{training_dir}/discriminator_model/weight_iter_{self.training_step}.pt")


def calculate_loss(network_output, label):
    return nn.functional.mse_loss(network_output["value"], label, reduction='none').mean()
...
def train(model, training_dir, data_loader, start_iter, end_iter):
    ...
    for i in range(1, py.get_training_step() + 1):
        ...
        add_training_info(training_info, 'loss_value', loss.item())
        loss.backward()
    ...
    model.save_model(training_dir)
    print("Discriminator_Optimization_Done", model.training_step, flush=True)
    eprint("Discriminator_Optimization_Done", model.training_step)
...
if __name__ == '__main__':
    ...
    data_loader = DiscriminatorDataLoader(conf_file_name)
    model = Model()
